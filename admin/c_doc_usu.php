<?php
		...
		$nombre='';$tramite='';$tipodoc='';$fecha='';$hora='';$folios='';$ndoc='';$procedencia='';$dependencia='';$asunto='';$descripcion='';
		if(!empty($_GET['id'])){	
			$id=$_GET['id'];
			$can=mysql_query("SELECT * FROM flujo where id=$id");
...
<blockquote>
 <a href="l_doc_usu.php" class="btn btn-info"><i class="icon-list-alt"></i> Ir Listado de Documentos</a>
 </blockquote>
<div align="center">
...
<table width="40%" border="0">
  <tr>
    <td colspan="3"><?php
		...
		$descripcion=$_POST['descripcion'];
			...
			if($boton=='Actualizar'){
				$xSQL="Update flujo Set idexpediente='$nombre',fecha='$fecha',hora='$hora',tipodoc='$tipodoc',ndoc='$ndoc',folios='$folios',procedencia='$procedencia',dependencia='$dependencia',asunto='$asunto',descripcion='$descripcion',area='$area',folios='$folios',remite='$remite'
							Where id=$id";
				mysql_query($xSQL);
			...
			//REGISTRAR DATOS DEL DOCUMENTO		
					...
					//TABLA EXPEDIENTE
						$sql = "INSERT INTO expediente (idexpediente,tramite,fecha,hora,tipodoc,ndoc,folios,procedencia,dependencia,asunto,descripcion,area,estado) VALUES ('$nombre','$tramite','$fecha','$hora','$tipodoc','$ndoc','$folios','$procedencia','$dependencia','$asunto','$descripcion','12','s')";
						//TABLA FLUJO
					$sql2 = "INSERT INTO flujo (idexpediente,tramite,fecha,hora,tipodoc,ndoc,folios,procedencia,dependencia,asunto,descripcion,area,estado) VALUES ('$nombre','$tramite','$fecha','$hora','$tipodoc','$ndoc','$folios','$procedencia','$dependencia','$asunto','$descripcion','12','s')";
						
						mysql_query($sql);
						mysql_query($sql2);
						$nombre='';$tramite='';$fecha='';$hora='';$tipodoc='';$ndoc='';$folios='';$procedencia='';$dependencia='';$asunto='';$descripcion='';$ndoc='';
						echo '	<div class="alert alert-success">
								  <button type="button" class="close" data-dismiss="alert">X</button>
  ...
  <tr>
    <td><strong>Fecha Recepion</strong></td>
    <td colspan="2">
		<?php 
			if(!empty($fecha)){ 
...
$query=mysql_query("select * from tipo_documento order by nombre asc");
  ...
  <tr>
    ...
    <td colspan="2">
    		<?php 
			if(!empty($asunto)){ 
				echo '<input type="text" name="asunto" id="remite" value='.$asunto.' required>';
			}else{
				echo '<input type="text" name="asunto" id="asunto" required>';	
			}
		?>
    </td>
  </tr>
