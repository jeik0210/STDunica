<?php
		...
		if(!$_SESSION['tipo_usu']=='a'){
			...
			if(!empty($_GET['estado'])){
				$codig=$_GET['estado'];
				$cans=mysql_query("SELECT * FROM  expediente WHERE estado='s' and id=$codig and area='$sarea'");
				if($d=mysql_fetch_array($cans)){
					$estado='n';
				}else{
					$estado='s';
				}
				$sql="UPDATE expediente SET estado='$estado' WHERE id=$codig and area='$sarea'";
				mysql_query($sql);
			}
		}
?>
...
<form name="form1" method="post" action="">
	<a href="c_doc_usu.php" class="btn btn-info"><i class="icon-user"></i> Nuevo Documento</a>
...
<td><strong>Acciones</strong></td>
  ...
  <?php
  	...
  	if(empty($_POST['bus'])){
		$can=mysql_query("SELECT * FROM expediente where area='$sarea' ");
	}else{
		$buscar=$_POST['bus'];
		$can=mysql_query("SELECT * FROM expediente where area='$sarea' and idexpediente LIKE '$buscar%'");
	}
	$i=0;
  ...
  <tr>
	...
	if($diast<=1)
	{
	echo $dato['idexpediente']." "."("."<font color='#0000FF'>"."<strong>".dias_transcurridos($fecha_i,$fechaf) ." "."Dias"."</font>"."</strong>".")"
	...
	;
	 ...
	 ?></td>
    <td><a href="../admin/l_doc3.php?estado=<?php echo $dato['id']; ?>"><?php echo $estado; ?></a></td>
    <td><?php 
	$area=$dato['area'];
...
$fila=mysql_fetch_array($query);	
    ...
    <td><?php 
	$tdoc=$dato['tipodocumento'];
	$sql="select * from tipo_documento where id=$tdoc";
$query=mysql_query($sql);
$fila=mysql_fetch_array($query);	
    ...
    <td><a href="c_doc_usu.php?id=<?php echo $dato['id']; ?>" title="Editar Documento" class="btn btn-info"><i class="icon-pencil"></i></a></td>
  </tr>
  <?php } ?>
  </tbody>
</table>
