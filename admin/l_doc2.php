<?php
		...
		if(!$_SESSION['tipo_usu']=='a'){
			...
			if(!empty($_GET['estado'])){
				$codig=$_GET['estado'];
				$cans=mysql_query("SELECT * FROM  flujoe WHERE estado='s' and id=$codig and area='$sarea'");
				if($d=mysql_fetch_array($cans)){
					$estado='n';
				}else{
					$estado='s';
				}
				$sql="UPDATE flujoe SET estado='$estado' WHERE id=$codig and area='$sarea'";
				mysql_query($sql);
			}
		}
?>
...
<form name="form1" method="post" action="">
	<a href="c_doc2.php" class="btn btn-info"><i class="icon-user"></i> Nuevo Documento</a>
...
<td><strong>Acciones</strong></td>
  ...
  <?php
  	...
  	if(empty($_POST['bus'])){
		$can=mysql_query("SELECT * FROM flujoe where area='$sarea' and codep='$sdep' ");
	}else{
		$buscar=$_POST['bus'];
		$can=mysql_query("SELECT * FROM flujoe where area='$sarea' and codep='$sdep' and idexpediente='$buscar' or tramite='$buscar'");
	}
	$i=0;
  ...
  <tr>
	...
	;
	 ...
	 ?></td>
    <td><a href="../admin/l_doc2.php?estado=<?php echo $dato['id']; ?>"><?php echo $estado; ?></a></td>
    <td><?php 
...
$fila=mysql_fetch_array($query);
    ...
    <td><a href="c_doc2.php?id=<?php echo $dato['id']; ?>" title="Editar Documento" class="btn btn-info"><i class="icon-pencil"></i></a></td>
  </tr>
  <?php } ?>
  </tbody>
</table>
